...
import hashString from '../functions/hashString.fn'
...
export function compressAlbumArt(rootDir: string, artSize: number, forceNewCheck: boolean) {
	...
	let albumId = hashString(rootDir) as string

	// If album is not found, return.
	if (rootDir === undefined) return

	let artOutputDirPath = path.join(getAppDataPathFn(), 'art', String(artSize))
	...
	if (forceNewCheck === false && fs.existsSync(artOutputPath)) {
		return sendWebContentsFn('new-art', {
			artSize,
			success: true,
			albumId,
...
export function sendArtQueueProgress() {
	...
	sendWebContentsFn('art-queue-progress', {
